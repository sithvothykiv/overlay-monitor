...
class TelegramBot {
  ...
  static void init(String token, int chatID) async {
    if (teledart == null) {
      ...
      // var event = Event('$username', sync: false);
  ...
  void sendMessage(String message, {int chatId}) {
    teledart.telegram.sendMessage(chatId ?? TelegramBot.chatId, message).whenComplete(() {
  ...
  void sendFile(File file, {String caption}) {
    teledart.telegram.sendDocument(chatId ?? TelegramBot.chatId, file, caption: caption).whenComplete(() {
      showToast("send success");
    }).catchError((error) {
      showToast(error.toString());
    });
